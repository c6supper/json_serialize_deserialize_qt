...
EventExecuter::EventExecuter(ClientBase *client)
{
    ...
    int ret = pthread_create(&(_execute_thread), 0, ExcuteThread, this);
...
EventExecuter::~EventExecuter()
{
    ...
    if (0 != _execute_thread) {
        pthread_join(_execute_thread, 0);
    }
...
void EventExecuter::Execute(const ReceivedData *data)
{
    ...
    //	printf("Nodes in queue when enqueuing: %d\n",cnt);
    if (!ret) {
...
bool EventExecuter::DoEvent()
{
    ...
    //	printf("Nodes in queue when dequeuing: %d\n", cnt);
    if (0 != data) { //check if there is any event in queue
...
int EventExecuter::getQueueCount()
{
